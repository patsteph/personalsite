...
// Initialize Firebase Admin (ensure it's done)
initializeAdminApp();
...
export default async function handler(
  ...
  try {
    ...
    console.error('Google Books API auth error:', error);
  ...
  if (!apiKey) {
    console.error('Google Books API Key is missing from environment variables.');
  ...
  if (isbn && typeof isbn === 'string') {
    ...
    return res.status(400).json({ success: false, error: 'Missing query parameter: requires isbn or q' });
  }

  try {
    console.log(`Querying Google Books API: ${googleApiUrl.replace(apiKey, '***')}`); // Don't log the key
    ...
    if (!googleRes.ok) {
      ...
      throw new Error(`Google Books API request failed with status ${googleRes.status}`);
    }
