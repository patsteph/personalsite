...
async function handleAdminBlog(
  ...
  if (req.method === "GET" && req.query.admin) {
    try {
      console.log(
        `Admin request: Fetching all documents from ${BLOG_COLLECTION}`,
      );
      const querySnapshot = await postsCollection
        .orderBy("updatedAt", "desc")
        .get(); // Fetch all, order by update time
      ...
      const errorMessage =
        error.code === "FAILED_PRECONDITION" && error.message.includes("index")
    ...
    try {
      ...
      const postData = {
        ...
        publishedAt: req.body.published
          ? Timestamp.fromDate(new Date(req.body.publishedAt || now))
          : null,
      };
  ...
  if (req.method === "PUT") {
    try {
      ...
      if (!id || typeof id !== "string") {
        return res
          .status(400)
          .json({ success: false, error: "Blog post ID is required" });
      }
      ...
      const updateData = {
        ...
        ...(req.body.publishedAt && {
          publishedAt: Timestamp.fromDate(new Date(req.body.publishedAt)),
        }),
        ...(req.body.createdAt && {
          createdAt: Timestamp.fromDate(new Date(req.body.createdAt)),
        }),
      };
      ...
      if (!updatedDoc.exists) {
        return res
          .status(404)
          .json({ success: false, error: "Blog post not found after update" });
      }
      ...
      if (docSnap.exists) {
        ...
        if (postData?.published) {
          return res.status(200).json({
            ...
            data: {
              id: docSnap.id,
              ...convertFirestoreToApiResponse(postData),
            },
          });
        } else {
