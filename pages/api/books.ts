...
function convertFirestoreToApiResponse(
...
export default async function handler(
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      try {
        ...
        try {
          ...
          if (googleBooksId) {
            queryConditions.push(
              Filter.where("googleBooksId", "==", googleBooksId),
            );
          }
          ...
          if (queryConditions.length > 0) {
            ...
            if (!duplicateSnapshot.empty) {
              ...
              console.log(
                `Books API: Duplicate found (ID: ${duplicateDoc.id}) based on ISBN/GoogleBooksID.`,
              );
            ...
            console.log(
              "Books API: Skipping duplicate check as no ISBN or GoogleBooksID provided.",
            );
          }
        } catch (duplicateCheckError: any) {
          console.error(
            "Books API: ERROR DURING DUPLICATE CHECK:",
            duplicateCheckError,
          );
        ...
        const bookData = {
          ...
          createdAt: sanitizedBody.createdAt
            ? Timestamp.fromDate(new Date(sanitizedBody.createdAt))
            : now,
          updatedAt: now,
          // Convert finishedDate if provided
          ...(sanitizedBody.finishedDate && {
            finishedDate: Timestamp.fromDate(
              new Date(sanitizedBody.finishedDate),
            ),
          }),
        };
    ...
    if (req.method === "PUT") {
      try {
        ...
        if (updateData.hasOwnProperty("finishedDate")) {
          updateData.finishedDate = updateData.finishedDate
            ? Timestamp.fromDate(new Date(updateData.finishedDate))
            : null;
        }

        console.log(
          `Books API: Updating document ${id} with data:`,
          updateData,
        );
        ...
        if (!updatedDoc.exists) {
          console.error(
            `Books API: Book document ${id} not found after update.`,
          );
          return res
            .status(404)
            .json({ success: false, error: "Book not found after update" });
        }
    ...
    if (!res.headersSent) {
      ...
      console.error(
        "Books API: Headers already sent, could not send 500 response for uncaught error.",
      );
    }
