/**
 * Admin page for managing content items (signals)
...
export default function ContentItemsPage() {
  ...
  const [formData, setFormData] = useState({
    type: 'newsletter' as 'newsletter' | 'article',
    title: '',
    description: '',
    url: '',
    publisher: '',
  ...
  useEffect(() => {
    async function loadSignals() {
      ...
      try {
        ...
        console.log('Content items loaded:', data.data?.length || 0);
        
        setSignals(data.data || []);
      } catch (err: any) {
        console.error('Error loading content items:', err);
        setError(`Error loading content items: ${err.message}`);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Submitting new content item...');
      ...
      console.log('Content item created:', result.data?.id);
      ...
      // Reload signals
      const getResponse = await fetch('/api/content-items');
      const getData = await getResponse.json();
      setSignals(getData.data || []);
      
    } catch (err: any) {
      console.error('Error submitting content item:', err);
