...
export default function AdminBlogPage() {
  ...
  const handleSavePost = async (blogPost: BlogPost | Omit<BlogPost, "id">) => {
    console.log("handleSavePost: Called with blog post:", blogPost);

    try {
      if ("id" in blogPost && blogPost.id) {
        // Update existing post
        console.log(
          "handleSavePost: Updating existing post with ID:",
          blogPost.id,
        );
        const success = await api.blog.updateBlogPost(blogPost.id, blogPost);
        console.log("handleSavePost: Update result:", success);
        ...
        // Create new post
        console.log("handleSavePost: Creating new post");
        const newPost = await api.blog.addBlogPost(
          blogPost as Omit<BlogPost, "id">,
        );
        console.log("handleSavePost: Create result:", newPost);
      ...
      console.error("handleSavePost: Error saving blog post:", err);
      setError(
        `An error occurred while saving the blog post: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    }
  };
          ...
          {isEditorOpen ? (
            ...
            <>
              <div className="flex flex-col items-center mb-6">
                <h1 className="text-2xl font-bold text-steel-blue mb-4">
                  Manage Blog Posts
                </h1>
                <button
